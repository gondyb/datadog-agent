...
const (
	HttpServiceVerbosityNone int = iota
	HttpServiceVerbositySummary
	HttpServiceVerbosityVerbose
	HttpServiceVerbosityVeryVerbose
)

var (
	// Should be controlled by config
	HttpServiceVerbosity int = HttpServiceVerbosityVeryVerbose
)
...
func completeReqRespTracking(eventInfo *C.DD_ETW_EVENT_INFO, httpConnLink *HttpConnLink) {
	...
	if !connFound {
		...
		if HttpServiceVerbosity == HttpServiceVerbosityVeryVerbose {
	...
	if HttpServiceVerbosity == HttpServiceVerbosityVeryVerbose {
		...
		log.Infof("\n")
	} else if HttpServiceVerbosity == HttpServiceVerbosityVerbose {
...
func httpCallbackOnHTTPRequestTraceTaskParse(eventInfo *C.DD_ETW_EVENT_INFO) {
	...
	// http.httpTxAux.Url field need to be eliminated at some point
...
func httpCallbackOnHTTPRequestTraceTaskSrvdFrmCache(eventInfo *C.DD_ETW_EVENT_INFO) {
	...
	cacheEntry, found := httpCache[httpConnLink.http.Url]
	if !found {
...
func etwHttpServiceCallback(eventInfo *C.DD_ETW_EVENT_INFO) {
	...
	if HttpServiceVerbosity != HttpServiceVerbosityNone {
	...
	transferedETWDataLength += (uint64(eventInfo.event.userDataLength) + 0x68)
